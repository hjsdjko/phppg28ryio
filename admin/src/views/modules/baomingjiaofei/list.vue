<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"0px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"1px solid #ceddee","minHeight":"100px","padding":"20px 20px","boxShadow":"0 2px 3px 0px rgba(100,100,100,.05)","margin":"0 0 20px","overflow":"hidden","borderRadius":"20px 20px","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230810/3647a571588849539f384f974d3e6e28.jpg) no-repeat center top / cover","display":"flex","fontSize":"inherit","justifyContent":"center"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"auto","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('baomingjiaofei','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('baomingjiaofei','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu9" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('baomingjiaofei','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						批量支付
					</el-button>

					<el-button class="btn18" v-if="isAuth('baomingjiaofei','课程总费用统计')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('baomingjiaofei','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('baomingjiaofei','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan8" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('baomingjiaofei','签到')" @click="qiandaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','你已签到成功')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								签到
							</el-button>
							<el-button class="btn8" v-if="isAuth('baomingjiaofei','请假')" @click="xueshengqingjiaCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0 0 4px","fontSize":"inherit","color":"inherit","height":"auto"}'></span>
								请假
							</el-button>
							<el-button class="edit" v-if=" isAuth('baomingjiaofei','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('baomingjiaofei','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<xueshengqingjia-cross-add-or-update v-if="xueshengqingjiaCrossAddOrUpdateFlag" :parent="this" ref="xueshengqingjiaCrossaddOrUpdate"></xueshengqingjia-cross-add-or-update>
...
import xueshengqingjiaCrossAddOrUpdate from "../xueshengqingjia/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				xueshengqingjiaCrossAddOrUpdateFlag: false,
		...
		components: {
			...
			xueshengqingjiaCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			qiandaoxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','baomingjiaofei');
			...
			xueshengqingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptPay=='是'&&row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.xueshengqingjiaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','baomingjiaofei');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.xueshengqingjiaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.xueshengqingjiaCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				this.$storage.set('paytable','baomingjiaofei');
...
//统计接口
	...
	baomingjiaofeistatusChange(e,row){
		if(row.status==0){
