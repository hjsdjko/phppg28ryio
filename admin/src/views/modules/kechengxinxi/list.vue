<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"0px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"1px solid #ceddee","minHeight":"100px","padding":"20px 20px","boxShadow":"0 2px 3px 0px rgba(100,100,100,.05)","margin":"0 0 20px","overflow":"hidden","borderRadius":"20px 20px","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230810/3647a571588849539f384f974d3e6e28.jpg) no-repeat center top / cover","display":"flex","fontSize":"inherit","justifyContent":"center"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","margin":"0 0px 0px 0","borderRadius":"0px","alignItems":"center","background":"none","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"center","order":"2"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="课程类型" prop="kechengleixing">
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="舞种类型" prop="wuzhongleixing">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">舞种类型</label>
						<el-select clearable v-model="searchForm.wuzhongleixing" placeholder="请选择舞种类型" >
							<el-option v-for="(item,index) in wuzhongleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="教师姓名" prop="jiaoshixingming">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">教师姓名</label>
						<el-select clearable v-model="searchForm.jiaoshixingming" placeholder="请选择教师姓名" >
							<el-option v-for="(item,index) in jiaoshixingmingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('kechengxinxi','报名缴费')" @click="baomingjiaofeiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<baomingjiaofei-cross-add-or-update v-if="baomingjiaofeiCrossAddOrUpdateFlag" :parent="this" ref="baomingjiaofeiCrossaddOrUpdate"></baomingjiaofei-cross-add-or-update>
...
import baomingjiaofeiCrossAddOrUpdate from "../baomingjiaofei/add-or-update";
	export default {
		data() {
			return {
				...
				baomingjiaofeiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			baomingjiaofeiCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			baomingjiaofeiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.baomingjiaofeiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.baomingjiaofeiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.baomingjiaofeiCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.wuzhongleixing!='' && this.searchForm.wuzhongleixing!=undefined){
            params['wuzhongleixing'] = this.searchForm.wuzhongleixing
          }
