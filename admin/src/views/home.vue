...
<div class="content" :style='{"minHeight":"calc(100vh - 200px)","padding":"30px","margin":"0 auto","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230831/511269159d614e62a63378dfef0a7567.png) no-repeat center top / cover","display":"flex","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
	...
	<div :style='{"padding":"0","boxShadow":"0 0px 0px 0px rgba(100,100,100,.05)","margin":"0 0px 30px 0px","borderColor":"#fceaee","alignItems":"center","color":"#666","display":"flex","justifyContent":"center","borderRadius":"3px","flexWrap":"wrap","background":"none","borderWidth":"0px","width":"100%","fontSize":"18px","borderStyle":"double","order":"1"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"0 10px 0 0","margin":"0 2% 10px 0","borderColor":"#ceddee","borderRadius":"3px","textAlign":"center","background":"#2aca76","borderWidth":"0px","display":"flex","width":"18%","borderStyle":"solid","justifyContent":"center"}' v-if="isAuth('baomingjiaofei','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"32px","color":"#fff","fontWeight":"bold","height":"24px"}'>{{baomingjiaofeiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"#fff","height":"24px"}'>报名缴费总数</div>
			</div>
		</div>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"0 10px 0 0","margin":"0 2% 10px 0","borderColor":"#ceddee","borderRadius":"3px","textAlign":"center","background":"#ea8556","borderWidth":"0px","display":"flex","width":"18%","borderStyle":"solid","justifyContent":"center"}' v-if="isAuth('qiandaoxinxi','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","background":"none","justifyContent":"center","display":"flex","height":"100px"}'>
				<span class="icon iconfont icon-tongji5" :style='{"color":"#fff","fontSize":"32px"}'></span>
			</div>
			<div :style='{"width":"auto","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"32px","color":"#fff","fontWeight":"bold","height":"24px"}'>{{qiandaoxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"#fff","height":"24px"}'>签到信息总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 2 -->
	<div class="type2" :style='{"padding":"0px","margin":"0","display":"flex","justifyContent":"space-between","alignContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"100%","position":"relative","height":"auto","order":"1"}'>
		<div id="baomingjiaofeiChart1" class="echarts1" v-if="isAuth('baomingjiaofei','首页统计')"></div>
		<div id="qiandaoxinxiChart1" class="echarts2" v-if="isAuth('qiandaoxinxi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getbaomingjiaofeiCount();
		this.baomingjiaofeiChat1();
		this.getqiandaoxinxiCount();
		this.qiandaoxinxiChat1();
	},
	methods:{
		// 词云
		...
		getbaomingjiaofeiCount() {
			this.$http({
				url: `baomingjiaofei/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.baomingjiaofeiCount = data.data
				}
			})
		},
		baomingjiaofeiChat1() {
        ...
        var baomingjiaofeiChart1 = echarts.init(document.getElementById("baomingjiaofeiChart1"),'macarons');
        this.$http({
            url: `baomingjiaofei/value/kechengmingcheng/zongfeiyong`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                baomingjiaofeiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    baomingjiaofeiChart1.resize();
                };
            }
        });
      })
		...
		getqiandaoxinxiCount() {
			this.$http({
				url: `qiandaoxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.qiandaoxinxiCount = data.data
				}
			})
		},
		qiandaoxinxiChat1() {
        ...
        var qiandaoxinxiChart1 = echarts.init(document.getElementById("qiandaoxinxiChart1"),'macarons');
        this.$http({
            url: "qiandaoxinxi/group/xueyuanzhanghao",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].xueyuanzhanghao);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].xueyuanzhanghao
                    })
                }
                var option = {};
                ...
                // 使用刚指定的配置项和数据显示图表。
                qiandaoxinxiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    qiandaoxinxiChart1.resize();
                };
            }
        });
      })
