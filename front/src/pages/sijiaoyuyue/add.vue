...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="课程名称" prop="zhuanyewuzhong">
            <el-input v-model="ruleForm.zhuanyewuzhong" 
                placeholder="课程名称" clearable :disabled=" false  ||ro.zhuanyewuzhong"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="学时" prop="xueshi">
            <el-select v-model="ruleForm.xueshi" placeholder="请选择学时" :disabled=" false  ||ro.xueshi" >
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="学时费用" prop="xueshifeiyong">
			<el-input-number v-model="ruleForm.xueshifeiyong" placeholder="学时费用" :readonly="ro.xueshifeiyong"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="课程费用" prop="kechengfeiyong">
              <el-input v-model="kechengfeiyong" placeholder="课程费用" disabled></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`sijiaoyuyue/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('sijiaoyuyue/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`sijiaoyuyue/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
