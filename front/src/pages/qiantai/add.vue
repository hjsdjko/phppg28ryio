...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="前台账号" prop="qiantaizhanghao">
            <el-input v-model="ruleForm.qiantaizhanghao" 
                placeholder="前台账号" clearable :disabled=" false  ||ro.qiantaizhanghao"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="前台姓名" prop="qiantaixingming">
            <el-input v-model="ruleForm.qiantaixingming" 
                placeholder="前台姓名" clearable :disabled=" false  ||ro.qiantaixingming"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`qiantai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('qiantai/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`qiantai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
