...
session_start();
class XueyuanController extends CommonController {
    ...
    public function login(){
        ...
        $sql = "select * from `xueyuan` where `xueyuanzhanghao` = '".$username."' and `mima` = '".$password."'";
    ...
    public function register(){
        ...
        $colum = "xueyuanzhanghao";
        $trues = "select * from `xueyuan` where `xueyuanzhanghao` = '".$postData[$colum]."'";
        ...
        if($result->num_rows<1){
			...
			$key = implode(',',$keyArr);
			$v = implode(',',$valArr);
            $sql = "INSERT INTO `xueyuan` (`id`,".$key.") VALUES (".time().",".$v.")";
            $result = table_sql($sql);
    ...
    public function resetPass(){
        ...
        $counts = "select * from `xueyuan` where xueyuanzhanghao = '".$username."'";
        $cotte = table_sql($counts);
        if($cotte->num_rows<1){
            exit(json_encode(['code'=>500,'msg'=>"用户名错误。"]));
        }
        $sql = "update xueyuan set mima = '123456' where xueyuanzhanghao = '".$username."'";
        $result = table_sql($sql);
    ...
    public function page(){
        ...
        if ($base['isAdmin']!=1 || ($base['isAdmin']==1 && $base['tablename'] != "users")){
            ...
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == '${column.authTable}'){
                    $orsign = false;
                    if(!$orsign) {
                        $where .= " and `xueyuanzhanghao` = '".$_SESSION[$md5]."'";
                    }
                }
            }
        }
        
        $sql = "select * from `xueyuan` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $lists = "select * from `xueyuan` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function list(){
        ...
        $page_count = ceil($numberCount/$limt);//页数
        $startCount = ($page-1)*$limt;
        $lists = "select * from `xueyuan` ".$where." order by ".$sortorders." limit ".$startCount.",".$limt;
        $result = table_sql($lists);
        $arrayData = array();
        if ($result->num_rows > 0) {
            while ($datas = $result->fetch_assoc()){
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
            $xueyuanzhanghao = $postData['xueyuanzhanghao'];
            $resultInfo = table_sql("select xueyuanzhanghao from `xueyuan` where `xueyuanzhanghao` = '".$xueyuanzhanghao."'");
            if ($resultInfo->num_rows > 0) {
                exit(json_encode(['code'=>500,'msg'=>"学员账号已存在"]));
            }
            foreach ($postData as $key => $value){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
        $sql = "INSERT INTO `xueyuan` (`id`,".$k.") VALUES (".time().",".$v.")";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function add(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
        $sql = "INSERT INTO `xueyuan` (".$k.") VALUES (".$v.")";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function update(){
        ...
        $postData = json_decode($tmpData,true);
            $xueyuanzhanghao = $postData['xueyuanzhanghao'];
            $id = $postData['id'];
            $resultInfo = table_sql("select xueyuanzhanghao from `xueyuan` where id <> ".$id." and `xueyuanzhanghao` = '".$xueyuanzhanghao."'");
            if ($resultInfo->num_rows > 0) {
                exit(json_encode(['code'=>500,'msg'=>"学员账号已存在"]));
            }
        $v = array();
        ...
        $value = implode(',',$v);
        $sql = "UPDATE xueyuan SET ".$value." where id = ".$id;
        $result = table_sql($sql);
    ...
    public function delete(){
        ...
        $sql = "delete from xueyuan WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        $sql = "select * from `xueyuan` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            while($row = $result->fetch_assoc()) {
    ...
    public function value(){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM xueyuan ".$where." group by ".$xColumnName;
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM xueyuan ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM xueyuan ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM xueyuan ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function valueMul(){
        ...
        foreach(explode(",", $yColumnName) as $item){
            $sql = "SELECT ".$xColumnName.",sum(".$item.") total FROM xueyuan ".$where." group by ".$xColumnName." LIMIT 10";
            if(urldecode($request[6]) == '日') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$item.") total FROM xueyuan ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') LIMIT 10";
            }
            if(urldecode($request[6]) == '月') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$item.") total FROM xueyuan ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m') LIMIT 10";
            }
            if(urldecode($request[6]) == '年') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$item.") total FROM xueyuan ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y') LIMIT 10";
            }
            $result = table_sql($sql);
    ...
    public function group($columnName){
        ...
        $tokens = json_decode(base64_decode($token),true);
        $where = " where 1 ";
        $sql = "SELECT ".$columnName.",count(".$columnName.") as total FROM xueyuan ".$where." GROUP BY ".$columnName." ORDER BY $columnName asc";
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            $total = array();
